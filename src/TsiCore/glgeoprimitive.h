...
public:
    ...
    void draw() const {
        ...
        glColor4ub(brush().color().redF(),brush().color().greenF(),brush().color().blueF(),brush().color().alphaF());
...
private:
    void bufferizeVertexArray() {
        m_vertexBuffer=(GLdouble*)realloc(m_vertexBuffer,m_points.size()*3);
        m_indexBuffer=(GLushort*)realloc(m_indexBuffer,m_points.size());
        int i=0;
