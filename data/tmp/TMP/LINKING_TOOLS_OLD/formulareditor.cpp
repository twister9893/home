...
FormularEditor::FormularEditor(QWidget *parent) :
    ...
    m_defaultFieldType = Integer;
    m_defaultFieldDimension = No;
    ...
    for(int i = 0; i < g_fieldTypes.size(); i++) {
        m_fieldTypeActions.append(typePopup->addAction(g_fieldTypes.at(i)));
    }
    addFieldButton->setMenu(typePopup);
    extern QStringList g_capacities;
    capacityBox->addItems(g_capacities);
...
void FormularEditor::slot_addField(QAction *type) {
    ...
    switch(typeIndex) {
        ...
        case Unused: {
            ...
            m_formularModel->setData(m_formularModel->index(position), QVariant((long long int)field), Qt::EditRole);
            break;
        }
        case Scalable: {
